...
func (c *vsClient) selectConfigs(ctx context.Context, vmMoRef *types.ManagedObjectReference) ([]vmConfig, error) {
	...
	cfgs, err := c.configInfo(ctx, tagObjs)
	if err != nil {
		return []vmConfig{}, err
	}
...
// Get only the configuration tags.
func (c *vsClient) configInfo(ctx context.Context, attachedTags []tags.Tag) ([]vmConfig, error) {
	hwTags := []vmConfig{}

	for _, t := range attachedTags {
		// Only select tag categories corresponding with configs we care about.
		hwProp, err := c.filteredConfigNames(ctx, t.CategoryID)
		if err != nil {
			return []vmConfig{}, err
		}
